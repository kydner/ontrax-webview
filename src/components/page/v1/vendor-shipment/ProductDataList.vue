            ...
            <template>
              <div v-if="['IN_TRANSIT'].includes(form.status)" class="tw-flex tw-items-center tw-space-x-6 tw-text-xs">
                <div class="tw-basis-auto tw-flex tw-items-center tw-space-x-2">
                  ...
                  <div class="tw-text-secondary-text">Qty Ordere</div>
                </div>
                <div>
                  {{ format(product.qtyOrder, { precision: 0 }) }}
                </div>
              </div>
              <span v-if="form.status === 'RECEIVED'" class="tw-text-xs">
                Qty Order: {{ format(product.qtyOrder, { precision: 0 }) }}
              </span>
              <!-- <plus-minus-field
          ...
          <q-card-section class="tw-p-2">
            <div class="tw-flex tw-items-center tw-justify-between">
              ...
              <div class="tw-basis-auto tw-text-right">
                <!-- <plus-minus-field
                  v-if="form.status === 'IN_TRANSIT'"
                  v-model="details[dialogIndex]."
                  :allow-increase="true"
                  :max="details[dialogIndex]?.qtyOrdered"
                  :disable="isDisable"
                  @click.stop
                />
                <plus-minus-field
                  v-else
                  v-model="details[dialogIndex].qtyOrdered"
                  :allow-increase="true"
                  :disable="isDisable"
                  @click.stop
                /> -->
