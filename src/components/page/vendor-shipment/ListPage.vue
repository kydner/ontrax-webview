<template>
  <scrollable-container :suffix-event="metaVendorShipment.name">
    ...
    >
      ...
      <slot :name="slotName" v-bind="(data as VendorShipmentResponsePage)" />
    </template>
      <!-- end-prettier-ignore -->
...
interface Props {
  keyName?: keyof VendorShipmentResponsePage
}
...
interface Slots<T> {
  ...
  'filter:left': () => VNode
  'filter:right': () => VNode
  'list:content': (props: { item: VendorShipmentResponsePage }) => VNode
...
defineSlots<Slots<VendorShipmentResponsePage>>()
