...
const state = reactive<ResponseState<VendorShipmentDetailResponse[]>>({
  isLoading: false,
  data: null,
  errorMessage: null,
})
...
const fetchData = async () => {
  try {
    ...
    state.data = response.map((product) => {
      ...
      } as VendorShipmentDetailResponse
    })
    // .filter((item) => item.vendorShipmentId !== undefined) as VendorShipmentDetailResponse[]
  } catch (error) {
...
const getProductId = (product: VendorShipmentDetailResponse) => product.productId

const updateQty = (product: VendorShipmentDetailResponse, qty: number) => {
...
const toggleItem = (product: VendorShipmentDetailResponse, checked: boolean) => {
  const current = [...details.value]
