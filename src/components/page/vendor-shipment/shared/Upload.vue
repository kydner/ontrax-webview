  ...
  >
    ...
    <template #default="{ tLabel }">
      <Field :name="props.name || tLabel" :label="tLabel" :rules="currentRules" :model-value="props.modelValue">
        ...
        <div v-if="allAttachments.length > 0" class="tw-mt-3 tw-flex tw-flex-col tw-gap-2">
          <div
            v-for="(file, index) in allAttachments"
            :key="file.id"
            class="tw-flex tw-items-center tw-justify-between tw-bg-gray-800 tw-p-2 tw-rounded"
          >
            ...
            <div class="tw-flex tw-items-center tw-gap-1">
              ...
              <span
                v-if="!disable"
                class="tw-relative tw-cursor-pointer tw-p-1"
                v-ripple
...
export interface KInputProps extends KLabelProps {
  ...
  inputClass?: string
  icon?: string
  attachmentInfo?: VendorShipmentUploadResponse[]
...
const allAttachments = computed(() => {
  const infoFromProps = props.attachmentInfo || []
...
const handleRemoveFile = (index: number) => {
  ...
  if (props.attachmentInfo?.some((attr) => attr.id === fileToRemove.id)) {
