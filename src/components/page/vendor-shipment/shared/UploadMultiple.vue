<template>
  <Field :name="props.name || label || 'upload-file'" :rules="currentRules" :model-value="props.modelValue">
    <q-file
      ref="fileInputRef"
    ...
    >
      ...
      <template v-else>
        ...
        <span class="tw-text-gray-400">Upload Files</span>
      </template>
    </div>

    <div v-if="allAttachments.length > 0" class="tw-mt-3 tw-flex tw-flex-col tw-gap-1">
      <div
        v-for="(file, index) in allAttachments"
        :key="file.id"
        class="tw-flex tw-items-center tw-justify-between tw-p-2 tw-py-0 tw-rounded"
      >
        <div class="tw-flex tw-items-center tw-gap-2">
          <q-icon name="insert_drive_file" color="warning" />
          <span class="tw-text-warning tw-text-xs">
            {{ truncate(file.filename, props.filenameMaxLength) }}
          </span>
          <q-tooltip>{{ file.filename }}</q-tooltip>
        </div>
        ...
        <div class="tw-flex tw-items-center tw-gap-1">
          ...
          />
          <span v-if="!disable" class="tw-relative tw-cursor-pointer tw-p-1" v-ripple @click="handleRemoveFile(index)">
    ...
    <ErrorMessage :name="props.name || label || 'upload-file'" class="tw-text-red-500 tw-text-xs tw-mt-1" />
...
import { KLabelSlots } from 'src/components/ui/KLabel.vue'
...
const emit = defineEmits<{
  ...
  (e: 'uploaded:local-file', value: VendorShipmentUploadResponse): void
}>()
...
// --- LOGIKA PREVIEW & DELETE ---
const localUploadedFiles = ref<VendorShipmentUploadResponse[]>([])
const removedAttachmentIds = ref<(id | string)[]>([])

const allAttachments = computed(() => {
  const infoFromProps = props.attachments || []
  const filteredProps = infoFromProps.filter((item) => !removedAttachmentIds.value.includes(item.id))
  return [...filteredProps, ...localUploadedFiles.value]
})

// Reset local state jika parent mengirim data shipment yang benar-benar baru
watch(
  () => props.attachments,
  () => {
...
watch(selectedFiles, async (files) => {
  ...
  try {
    ...
    const newIds: (id | string)[] = props.modelValue ? [...props.modelValue] : []
    ...
    for (const file of files) {
      ...
      localUploadedFiles.value.push(data)
      emit('uploaded:local-file', data)
      newIds.push(data.id)
    }
    emit('update:model-value', newIds)
  } catch (error) {
...
const handleRemoveFile = (index: number) => {
  ...
  const fileToRemove = allAttachments.value[index]
  if (!fileToRemove) return

  // 1. Emit ID yang tersisa ke Parent
  const updatedIds = [...props.modelValue].filter((id) => id !== fileToRemove.id)
  emit('update:model-value', updatedIds.length > 0 ? updatedIds : null)

  // 2. Jika itu file baru, hapus dari list lokal
  localUploadedFiles.value = localUploadedFiles.value.filter((f) => f.id !== fileToRemove.id)

  // 3. Jika itu file dari props, tambahkan ke list 'hidden' agar hilang dari UI
  if (props.attachments?.some((attr) => attr.id === fileToRemove.id)) {
    removedAttachmentIds.value.push(fileToRemove.id)
  }
}
